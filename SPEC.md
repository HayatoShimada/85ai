...
- **送信データ**: base64エンコードされた透過PNG（人物セグメンテーション済み）
- **フレームレート**: `MIRROR_FPS` 環境変数で設定（デフォルト15fps）
...
1. OpenCVでカメラフレーム取得
2. 左右反転（鏡像）
3. MediaPipe Selfie Segmentation (model_selection=0, 軽量モデル)
4. セグメンテーションマスクをアルファチャンネルに適用 (BGRA)
5. 透過PNGにエンコード → base64文字列で返却
...
| `MIRROR_WIDTH` | 640 | キャプチャ幅 |
| `MIRROR_HEIGHT` | 480 | キャプチャ高さ |
| `MIRROR_FPS` | 15 | 配信フレームレート |
...
| `MIRROR_WIDTH` | - | キャプチャ幅（デフォルト: `640`） |
| `MIRROR_HEIGHT` | - | キャプチャ高さ（デフォルト: `480`） |
| `MIRROR_FPS` | - | 配信フレームレート（デフォルト: `15`） |
...
| Pillow | 画像処理 |
| opencv-python-headless | カメラキャプチャ |
