...
def _select_backend() -> str:
    ...
    if SEGMENTER_BACKEND == "vision":
        ...
        if vision_segmenter.is_available():
            return "vision"
        print("Warning: Vision requested but not available, falling back to MediaPipe")
        return "mediapipe"
    ...
    def start(self) -> bool:
        ...
        if backend == "vision":
            try:
                ...
                print(f"Mirror started: camera={self._camera_index}, "
                      f"capture={actual_w}x{actual_h}@{actual_fps:.0f}fps, "
                      f"backend=Apple Vision (Neural Engine)")
            except Exception as e:
                print(f"Vision init failed: {e}, falling back to MediaPipe")
                backend = "mediapipe"
        ...
        print(f"  webp_q={WEBP_QUALITY}, blur={MASK_BLUR}, threshold={MASK_THRESHOLD}")
