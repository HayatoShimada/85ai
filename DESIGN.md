...
- ミラーフレーム: Backend(OpenCV+MediaPipe) → `WS /ws/projection/display` → プロジェクター
...
| AI推論 | Google Gemini API (`gemini-1.5-pro`) | 服装解析・提案生成 |
...
| ミラー映像 | OpenCV + MediaPipe Selfie Segmentation | 人物セグメンテーション |
| 画像処理 | Pillow | 画像デコード |
...
#### `WS /ws/mirror`
ミラーカメラの人物切り抜きフレームをbase64 PNGで配信。接続時に自動起動。
...
1. Mac StudioにUSBカメラ接続
2. OpenCVでフレーム取得 → 左右反転（鏡像）
3. MediaPipe Selfie Segmentation (model_selection=0) で人物セグメンテーション
4. アルファチャンネル適用 → 透過PNGエンコード → base64配信
5. プロジェクション画面で前面オーバーレイ表示

### カメラ管理

- `GET /api/mirror/cameras`: サーバー接続カメラの一覧取得
